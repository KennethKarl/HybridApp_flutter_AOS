...
**우리 프로젝트에 적용한 변형 — suffix 명확 구분 추가**

AWS의 예외명 방식에 우리의 플랫폼 suffix를 결합하면 다음과 같다.
...
- `errorCode` : 우리 체계의 `{category}_{4자리}_{suffix}` — 로그 필터링·집계용
...
| 항목 | **우리 체계** | 카카오식 | Stripe식 | **AWS식+suffix (권장 참고)** |
|------|:-----------:|:-------:|:-------:|:---------------------------:|
| 플랫폼 구분 | ✅ suffix | ❌ | ❌ | ✅ suffix + errorType |
| 가독성 | 중 | 하 | 상 | 상 |
| 확장성 | 높음 | 중 | 중 | 높음 |
